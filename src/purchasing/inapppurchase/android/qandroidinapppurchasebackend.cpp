...
QAndroidInAppPurchaseBackend::QAndroidInAppPurchaseBackend(QObject *parent)
    : QInAppPurchaseBackend(parent), m_mutex(QMutex::Recursive),
      m_isReady(false) {
  ...
  m_javaObject =
      QAndroidJniObject("org/qtproject/qt5/android/purchasing/QtInAppPurchase",
                        "(Landroid/content/Context;J)V",
                        QtAndroid::androidActivity().object<jobject>(),
                        reinterpret_cast<jlong>(this));
  if (!m_javaObject.isValid()) {
    qWarning("Cannot initialize IAP backend for Android due to missing "
             "dependency: QtInAppPurchase class");
    return;
  }
}
...
void QAndroidInAppPurchaseBackend::initialize() {
  ...
  qDebug("Initializing backend; type=%d", (int)backendType());
  jint backendTypeInt = (int)backendType();
  m_javaObject.callMethod<void>("initializeConnection", "(I;)V",
                                backendTypeInt);
...
void QAndroidInAppPurchaseBackend::restorePurchases() {
  ...
  foreach (QString previouslyFinalizedUnlockable,
           previouslyFinalizedUnlockables) {
...
void
QAndroidInAppPurchaseBackend::queryProducts(const QList<Product> &products) {
  QMutexLocker locker(&m_mutex);
  ...
  for (int i = 0; i < products.size(); ++i) {
    ...
    if (m_productTypeForPendingId.contains(product.identifier)) {
      qWarning("Product query already pending for %s",
  ...
  jobjectArray productIds =
      environment->NewObjectArray(newProducts.size(), cls, 0);
  environment->DeleteLocalRef(cls);

  for (int i = 0; i < newProducts.size(); ++i) {
    QAndroidJniObject identifier =
        QAndroidJniObject::fromString(newProducts.at(i));
    environment->SetObjectArrayElement(productIds, i, identifier.object());
  }

  m_javaObject.callMethod<void>("queryDetails", "([Ljava/lang/String;)V",
                                productIds);
  environment->DeleteLocalRef(productIds);
}

void QAndroidInAppPurchaseBackend::queryProduct(
    QInAppProduct::ProductType productType, const QString &identifier) {
#if defined(QANDROIDINAPPPURCHASEBACKEND_DEBUG)
...
void
QAndroidInAppPurchaseBackend::setPlatformProperty(const QString &propertyName,
                                                  const QString &value) {
  QMutexLocker locker(&m_mutex);
  if (propertyName.compare(QStringLiteral("AndroidPublicKey"),
                           Qt::CaseInsensitive) == 0) {
    m_javaObject.callMethod<void>(
...
void
QAndroidInAppPurchaseBackend::registerQueryFailure(const QString &productId) {
#if defined(QANDROIDINAPPPURCHASEBACKEND_DEBUG)
  qDebug("Query failed for %s", qPrintable(productId));
#endif

  QMutexLocker locker(&m_mutex);
  QHash<QString, QInAppProduct::ProductType>::iterator it =
      m_productTypeForPendingId.find(productId);
...
void
QAndroidInAppPurchaseBackend::consumeTransaction(const QString &purchaseToken) {
#if defined(QANDROIDINAPPPURCHASEBACKEND_DEBUG)
  ...
  QMutexLocker locker(&m_mutex);
  m_javaObject.callMethod<void>(
      "consumePurchase", "(Ljava/lang/String;)V",
...
bool QAndroidInAppPurchaseBackend::transactionFinalizedForProduct(
    QInAppProduct *product) {
...
void QAndroidInAppPurchaseBackend::checkFinalizationStatus(
    QInAppProduct *product, QInAppTransaction::TransactionStatus status) {
  // Verifies the finalization status of an item based on the following logic:
  // 1. If the item is not purchased yet, do nothing (it's either never been
  // purchased, or it's a
  //    consumed consumable.
  // 2. If the item is purchased, and it's a consumable, it's unfinalized. Emit
  // a new transaction.
  //    Consumable items are consumed when they are finalized.
  // 3. If the item is purchased, and it's an unlockable, check the local cache
  // for finalized
  //    unlockable purchases. If it's not there, then the transaction is
  //    unfinalized. This means
  //    that if the cache gets deleted or corrupted, the worst-case scenario is
  //    that the transactions
  //    are republished.
  QHash<QString, PurchaseInfo>::iterator it =
      m_infoForPurchase.find(product->identifier());
...
#if defined(QANDROIDINAPPPURCHASEBACKEND_DEBUG)
    qDebug("Product unfinalized: %s. Emitting transaction with status %d.",
           qPrintable(product->identifier()), status);
#endif

    QAndroidInAppTransaction *transaction = new QAndroidInAppTransaction(
        info.signature, info.data, info.purchaseToken, info.orderId, status,
        product, info.timestamp, QInAppTransaction::NoFailure, QString(), this);
...
void QAndroidInAppPurchaseBackend::registerProduct(const QString &productId,
...
#if defined(QANDROIDINAPPPURCHASEBACKEND_DEBUG)
  qDebug("Registering product %s with price %s", qPrintable(productId),
         qPrintable(price));
#endif
  ...
  QAndroidInAppProduct *product = new QAndroidInAppProduct(
      this, price, title, description, it.value(), it.key(), this);
...
void QAndroidInAppPurchaseBackend::registerPurchased(
    const QString &identifier, const QString &signature, const QString &data,
    const QString &purchaseToken, const QString &orderId,
    const QDateTime &timestamp) {
#if defined(QANDROIDINAPPPURCHASEBACKEND_DEBUG)
  qDebug("Registering previously purchased product: %s",
         qPrintable(identifier));
#endif

  QMutexLocker locker(&m_mutex);
  m_infoForPurchase.insert(
      identifier,
      PurchaseInfo(signature, data, purchaseToken, orderId, timestamp));
}
...
void QAndroidInAppPurchaseBackend::handleActivityResult(
    int requestCode, int resultCode, const QAndroidJniObject &data) {
  ...
  m_javaObject.callMethod<void>(
      "handleActivityResult", "(IILandroid/content/Intent;Ljava/lang/String;)V",
      requestCode, resultCode, data.object<jobject>(),
      QAndroidJniObject::fromString(product->identifier()).object<jstring>());
}

void
QAndroidInAppPurchaseBackend::purchaseProduct(QAndroidInAppProduct *product) {
#if defined(QANDROIDINAPPPURCHASEBACKEND_DEBUG)
  ...
  QMutexLocker locker(&m_mutex);
  if (!m_javaObject.isValid()) {
    purchaseFailed(product, QInAppTransaction::ErrorOccurred,
                   QStringLiteral("Java backend is not initialized"));
    return;
  }

  QAndroidJniObject intentSender = m_javaObject.callObjectMethod(
      "createBuyIntentSender",
      "(Ljava/lang/String;)Landroid/content/IntentSender;",
      QAndroidJniObject::fromString(product->identifier()).object<jstring>());
  if (!intentSender.isValid()) {
    purchaseFailed(product, QInAppTransaction::ErrorOccurred,
                   QStringLiteral("Unable to get intent sender from service"));
    return;
  }
...
void QAndroidInAppPurchaseBackend::purchaseFailed(QInAppProduct *product,
  ...
  QInAppTransaction *transaction = new QAndroidInAppTransaction(
      QString(), QString(), QString(), QString(),
      QInAppTransaction::PurchaseFailed, product, QDateTime(),
      QInAppTransaction::FailureReason(failureReason), errorString, this);
  emit transactionReady(transaction);
}

void QAndroidInAppPurchaseBackend::purchaseSucceeded(
    int requestCode, const QString &signature, const QString &data,
    const QString &purchaseToken, const QString &orderId,
    const QDateTime &timestamp)

{
  ...
  m_infoForPurchase.insert(
      ...
      PurchaseInfo(signature, data, purchaseToken, orderId, timestamp));
  QInAppTransaction *transaction = new QAndroidInAppTransaction(
      signature, data, purchaseToken, orderId,
      QInAppTransaction::PurchaseApproved, product, timestamp,
      QInAppTransaction::NoFailure, QString(), this);
  emit transactionReady(transaction);
}
